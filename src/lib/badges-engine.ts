// ─────────────────────────────────────────────────────────────
// Badges engine — pure checker logic with no implicit Supabase
// dependency. Callers pass the Supabase client (browser or
// service-role); the engine reads from `badges`, `user_badges`,
// and a handful of related tables to compute which previously-
// unearned badges the user has now earned.
//
// The engine is import-safe from server routes; it does NOT
// insert into `user_badges`. The caller (the server route in
// `/api/badges/check`, post-Session-4G) is responsible for the
...
export const runBadgeChecks = async (
...
// Day-of-week is derived server-side via
// `new Date(created_at).getDay()` once badge awarding moves to
// the server route in Session 4G.
